...
/////////////////////Dual MCL//////////////////
  ...
  if(!private_nh_.searchParam("dual_normalizer_ita", param_key_name))
    private_nh_.param("dual_normalizer_ita", ita_, 0.5);
  else
    private_nh_.param(param_key_name.c_str(), ita_, 0.5);
...
{
  if(!kdt_)
    ROS_INFO("old kdt_ is NULL. Rebuilding density tree.");
  else
    ROS_INFO("old kdt_ is not NULL. Rebuilding density tree.");
...
inline void
MixmclNode::poseToSe3(const pf_vector_t& vec_p, kernel::se3& se3_p)
{
...
inline void
MixmclNode::se3ToPose(const kernel::se3& se3_p, pf_vector_t& vec_p)
{
  ...
  //assertion
  //tf::Quaternion q = tf::createQuaternionFromYaw(vec_p.v[2]);
  //assert(((double)q.w())==((double)se3_p.ori_.W()));
  //assert(((double)q.x())==((double)se3_p.ori_.X()));
  //assert(((double)q.y())==((double)se3_p.ori_.Y()));
  //assert(((double)q.z())==((double)se3_p.ori_.Z()));
}
...
/////////////////////Dual MCL//////////////////
    ...
    boost::shared_ptr<KernelCollection> tree = kcgrid_->getTree(feature.x, feature.y, feature.dist);
